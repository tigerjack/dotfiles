...
It should only modify the values of Spacemacs settings."
  ;; Default theme order (tracked). Overridden below if a local file exists.
  (defvar my-dotspacemacs-themes '(spacemacs-light spacemacs-dark))
  (let ((local-themes-file (expand-file-name "~/.config/spacemacs/local-themes.el")))
    (when (file-exists-p local-themes-file)
      (load local-themes-file)))
  ...
  (setq-default
   ...
   dotspacemacs-themes my-dotspacemacs-themes
...
you should place your code here."
  ;; overleaf
  ;; --- overleaf.el (fork, until merged upstream) ---
  (add-to-list 'load-path (expand-file-name (substitute-env-in-file-name "$MDIR_LINUX_DATA/vc/forks/overleaf.el")))
  (require 'overleaf)

  (setq overleaf-cookies (overleaf-read-cookies-from-firefox :profile "overleaf"))

  (with-eval-after-load 'latex                       ; AUCTeX (Spacemacs latex layer)
...
;;; project management
  ...
  ;; AUCTEX for -shell-escape
  ;; (add-hook 'TeX-mode-hook
  ;;           (lambda ()
  ;;             (setq TeX-command-extra-options "-shell-escape")))
  ;; For the nice features of latex-extra-mode, see
  ...
  ;; Used to skip environments is latex during flyspell
  ;; see https://tex.stackexchange.com/a/154540/80949
  ...
  (defun auctex-mode-flyspell-skip-myenv ()
    (save-excursion
      ...
      (let ((p (point))
            (count 0))
        (not (or (and (re-search-backward "\\\\begin{\\(tikzpicture\\|minted\\)}" nil t)
                      (> p (point))
                      (or (not (re-search-forward "^\\\\end{\\(tikzpicture\\|minted\\)}" nil t))
                          (< p (point))))
...
This function is called at the very end of Spacemacs initialization."
  (custom-set-variables
   ...
   '(org-format-latex-options
     '(:foreground default :background default :scale 1.6 :html-foreground "Black"
                   :html-background "Transparent" :html-scale 1.0 :matchers
                   ("begin" "$1" "$" "$$" "\\(" "\\[")))
   '(package-selected-packages
     '(ac-ispell ace-jump-helm-line ace-link ace-window adaptive-wrap
                 add-node-modules-path adoc-mode afternoon-theme aggressive-indent
                 alect-themes alert ample-theme ample-zen-theme anaconda-mode
                 anaphora anti-zenburn-theme anzu apropospriate-theme async auctex
                 auto-compile auto-complete auto-dictionary auto-highlight-symbol
                 auto-yasnippet autothemer avy badwolf-theme biblio biblio-core
                 bibtex-completion bind-key bind-map birds-of-paradise-plus-theme
                 bubbleberry-theme bundler busybee-theme cargo
                 cherry-blossom-theme chruby clang-format clean-aindent-mode
                 clues-theme cmake-mode coffee-mode
                 color-theme-sanityinc-solarized color-theme-sanityinc-tomorrow
                 column-enforce-mode common-lisp-snippets company company-anaconda
                 company-auctex company-c-headers company-go company-quickhelp
                 company-statistics company-web counsel counsel-gtags csv-mode
                 cyberpunk-theme cython-mode dakrone-theme darkburn-theme
                 darkmine-theme darkokai-theme darktooth-theme dash deferred
                 define-word deft diff-hl diminish disaster django-theme
                 dracula-theme drupal-mode dumb-jump ein elisp-slime-nav
                 emmet-mode epl esh-help eshell-prompt-extras eshell-z
                 espresso-theme eval-sexp-fu evil evil-anzu evil-args evil-ediff
                 ...
                 flycheck-rust flyspell-correct flyspell-correct-helm
                 fringe-helper fuzzy gandalf-theme ggtags gh-md git-commit
                 ...
                 haml-mode hc-zenburn-theme helm helm-ag helm-bibtex
                 helm-c-yasnippet helm-company helm-core helm-css-scss
                 helm-descbinds helm-flx helm-gitignore helm-gtags helm-make
                 ...
                 hl-todo ht htmlize hungry-delete hy-mode hydra iedit
                 impatient-mode import-js indent-guide inf-ruby inkpot-theme
                 ir-black-theme ivy jazz-theme jbeans-theme js-doc js2-mode
                 js2-refactor json-mode json-snatcher key-chord langtool
                 latex-extra light-soap-theme link-hint linum-relative
                 live-py-mode livid-mode log4e lorem-ipsum lush-theme lv macrostep
                 madhat2r-theme magit magit-gitflow magit-popup magit-section
                 majapahit-theme markdown-mode markdown-toc markup-faces
                 material-theme minimal-theme minitest mmm-mode moe-theme
                 molokai-theme monochrome-theme monokai-theme move-text multi-term
                 multiple-cursors mustang-theme mwim naquadah-theme neotree
                 ...
                 org-ref organic-green-theme orgit overleaf packed paradox
                 parent-mode parsebib pcre2el pdf-tools persp-mode
                 phoenix-dark-mono-theme phoenix-dark-pink-theme php-extras
                 php-mode phpunit pip-requirements pkg-info planet-theme polymode
                 popup popwin pos-tip powerline prettier-js professional-theme
                 projectile pug-mode purple-haze-theme py-isort pyenv-mode pytest
                 pythonic pyvenv racer railscasts-theme rainbow-delimiters rake
                 rbenv rebecca-theme request restart-emacs reverse-theme robe
                 rspec-mode rubocop ruby-test-mode ruby-tools rust-mode rvm s
                 sass-mode scss-mode seti-theme shell-pop simple-httpd skewer-mode
                 slim-mode slime slime-company smartparens smeargle smyx-theme
                 soft-charcoal-theme soft-morning-theme soft-stone-theme
                 solarized-theme soothe-theme spacegray-theme spaceline sphinx-doc
                 spinner subatomic-theme subatomic256-theme sublime-themes
                 sunny-day-theme swiper tablist tagedit tango-2-theme
                 tango-plus-theme tangotango-theme tao-theme tern toc-org
                 toml-mode toxi-theme transient twilight-anti-bright-theme
                 ...
                 web-completion-data web-mode websocket which-key white-sand-theme
                 winum with-editor ws-butler xterm-color yaml-mode yapfify
                 yasnippet zen-and-art-theme zenburn-theme))
   '(paradox-automatically-star nil)
   '(paradox-github-token t)
   '(safe-local-variable-values
     '((TeX-command-extra-options . "-shell-escape") (javascript-backend . tern)
       (javascript-backend . lsp)
       (eval org-babel-do-load-languages 'org-babel-load-languages
             '((emacs-lisp) (calc . t) (C . t) (ditaa . t)))
       (eval org-babel-do-load-languages 'org-babel-load-languages
             '((emacs-lisp) (calc . t) (c . t) (ditaa . t)))
       (eval org-babel-do-load-languages 'org-babel-load-languages
             '((emacs-lisp) (calc . t) (ditaa . t)))))
